...
public class LevelGenerationAiServiceImpl implements LevelGenerationAiService {
     ...
     * 创建模拟选项
     */
    private java.util.List<LevelOption> createMockOptions() {
        java.util.List<LevelOption> options = new java.util.ArrayList<>();

        // 正确选项
        LevelOption option1 = new LevelOption();
        option1.setOptionName("Spring Boot框架");
        option1.setTrueAnswer(true);
        options.add(option1);

        LevelOption option2 = new LevelOption();
        option2.setOptionName("MySQL数据库");
        option2.setTrueAnswer(true);
        options.add(option2);

        // 干扰选项
        LevelOption option3 = new LevelOption();
